...
#include <netdb.h>
    ...
    transport_i* https_transport::accept()
    {
        ...
        snprintf(debug_file_name, 255, "rcv_sock-%d.raw", file_idx++);
        peer->debug_stream = fopen(debug_file_name, "wb");
#endif
        return peer;
    }
...
#if DEBUG_RAW
        if (status > 0) {
            fwrite(data, status, 1, debug_stream);
            fflush(debug_stream);
            printf("recv2: Flush log %d bytes\n", status);
        }
#endif
        return status;
    }
    ...
    int https_transport::recv(char* data, int size, long long time)
    {
        ...
        if (status > 0) {
            fwrite(data, status, 1, debug_stream);
            fflush(debug_stream);
            printf("recv3: Flush log %d bytes\n", status);
        }
#endif
        return status;
    }
    ...
    int https_transport::connect(const char* hostname, int port)
    {
        throw "ssl connect not implemented yet";
    }
